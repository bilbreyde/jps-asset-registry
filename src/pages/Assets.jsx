...
function Assets() {
  ...
  const clearFilters = () => { setSearchInput(''); setSearch(''); setFilterDept(''); };
  ...
  const handleSubmit = async (data) => {
    try {
      if (editing) { await editAsset(editing.id, data); toast('Asset updated successfully'); }
      else         { await addAsset(data);               toast('Asset added successfully'); }
      closeModal();
    } catch (err) { toast(err.message, 'error'); }
  };

  const handleDelete = async (id) => {
    try { await removeAsset(id); toast('Asset deleted'); }
    catch (err) { toast(err.message, 'error'); }
  };
  ...
  return (
    <>
        ...
        <div className="pagination-bar">
          ...
          <div className="pagination-controls">
            <label className="page-size-label">
              Per page:
              <select className="page-size-select" value={pageSize} onChange={e => setPageSize(Number(e.target.value))} aria-label="Records per page">
            ...
            <button className="page-btn" onClick={goPrev} disabled={currentPage === 0 || loading} aria-label="Previous page">← Prev</button>
            <span className="page-num">Page {currentPage + 1}</span>
            <button className="page-btn" onClick={goNext} disabled={!hasMore || loading} aria-label="Next page">Next →</button>
          </div>
        </div>

        {/* Table */}
        {loading
          ? <div className="loading-card"><div className="spin-ring" aria-label="Loading" role="status" /><p>Loading assets…</p></div>
      ...
      {/* Modal */}
      {showModal && (
        <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && closeModal()} role="presentation">
          <div className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title">
            <div className="modal-header">
              <h2 id="modal-title" className="modal-title">{editing ? 'Edit Asset' : 'Add New Asset'}</h2>
              <button className="modal-close" onClick={closeModal} aria-label="Close dialog"><CloseIcon /></button>
            </div>
